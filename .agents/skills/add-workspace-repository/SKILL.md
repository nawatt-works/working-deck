...
The other two combinations are errors that
`python3 _Mission-Control/tooling/repos_status.py` reports.
...
6. Use `$manage-repository-catalog` to add the catalog entry. Do not hand-edit `_Mission-Control/workspace-meta/repositories.yaml` in this skill.
7. Run `python3 _Mission-Control/tooling/repos_status.py` and confirm the new repository reports `external` or `internal` with no errors.
...
- Never add `AGENTS.md`, `CLAUDE.md`, `.agents/`, `.claude/`, `_Mission-Control/`, legacy `workspace-meta/`, or legacy `workbench/` content to a repository under `repos/`.
...
- Do not record a remote URL, description, or class inside `_Mission-Control/workspace-meta/repositories.yaml`; schema version 1 holds only `repo_id` and `path`.
